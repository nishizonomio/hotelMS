    ...
    switch ($method) {
        case 'GET':
            ...
            foreach ($guests as &$guest) {
                ...
                // Handle name field
                if ($hasFirstLastName) {
                    if (!$hasFullName || empty($guest['name'])) {
                        $guest['name'] = trim(($guest['first_name'] ?? '') . ' ' . ($guest['last_name'] ?? ''));
                    }
                }

                // Handle phone field
                if (isset($guest[$phoneField])) {
                    $guest['phone'] = $guest[$phoneField];
                } elseif (isset($guest['second_phone']) && empty($guest['phone'])) {
                    $guest['phone'] = $guest['second_phone'];
                }

                // Set defaults
                $guest['location'] = $guest['location'] ?? 'Unknown';
                $guest['loyalty_tier'] = $guest['loyalty_tier'] ?? 'bronze';
            }
        ...
        case 'POST':
            ...
            $stmt = $conn->prepare("SELECT * FROM guests WHERE $primaryKey = :id");
            $stmt->execute([':id' => $newId]);
            $newGuest = $stmt->fetch(PDO::FETCH_ASSOC);

            // Normalize for frontend
            if (isset($newGuest['guest_id'])) {
                $newGuest['id'] = $newGuest['guest_id'];
            }
            if ($hasFirstLastName && !$hasFullName) {
                $newGuest['name'] = trim(($newGuest['first_name'] ?? '') . ' ' . ($newGuest['last_name'] ?? ''));
            }
            if (isset($newGuest[$phoneField])) {
                $newGuest['phone'] = $newGuest[$phoneField];
            }
        ...
        case 'PUT':
            ...
            respond(['success' => true, 'message' => 'Guest updated successfully']);
            break;
        ...
        case 'DELETE':
            ...
            $stmt = $conn->prepare("DELETE FROM guests WHERE $primaryKey = :id");
